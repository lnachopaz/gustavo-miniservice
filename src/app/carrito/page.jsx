...
import {
  ...
  CreditCard, Banknote, Store, Bike, CheckCircle2,
...
const METODOS_PAGO = [
  {
    ...
    label: 'Mercado Pago',
    icon: '💳',
    desc: 'Tarjetas, QR, billetera virtual',
    badge: 'Recomendado',
  },
  {
    id: 'tarjeta',
    ...
    icon: '🏦',
    desc: 'Todas las tarjetas aceptadas',
    badge: null,
  },
  {
    id: 'transferencia',
  ...
  },
  {
    ...
    label: 'Efectivo',
    icon: '💵',
    desc: 'Pagás al retirar o al recibir',
    badge: null,
  },
];
...
const METODOS_ENTREGA = [
  ...
  },
  {
    ...
    desc: 'Consultá costo según zona',
  },
];
...
export default function CarritoPage() {
  ...
  const [paso, setPaso]               = useState(1); // 1: carrito, 2: datos, 3: confirmado
  const [metodoPago, setMetodoPago]   = useState('mercadopago');
  const [metodoEntrega, setEntrega]   = useState('retiro');
  const [loading, setLoading]         = useState(false);
  const [form, setForm] = useState({
    nombre: '', apellido: '', telefono: '', email: '', direccion: '', nota: ''
  });
      ...
      <div className="container-max py-16 text-center max-w-lg mx-auto">
        <div className="bg-white rounded-3xl p-10 shadow-sm border border-gray-100">
          ...
          <p className="text-gray-500 mb-2">
            Gracias <strong>{form.nombre}</strong>, recibimos tu pedido.
          </p>
          ...
          <div className="bg-brand-purple-50 rounded-2xl p-4 text-left mb-8">
            <p className="text-xs font-semibold text-brand-purple-600 uppercase tracking-wider mb-2">Resumen del pedido</p>
            <p className="text-sm text-gray-600">
              Método de pago: <strong>{METODOS_PAGO.find(m => m.id === metodoPago)?.label}</strong>
            </p>
            <p className="text-sm text-gray-600">
              Entrega: <strong>{METODOS_ENTREGA.find(m => m.id === metodoEntrega)?.label}</strong>
            </p>
          </div>
        ...
        {[
          ...
          <div key={n} className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
              paso >= n ? 'bg-brand-purple-800 text-brand-yellow-400' : 'bg-gray-200 text-gray-500'
            }`}>
              {n}
            </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {METODOS_ENTREGA.map(m => (
                    <button
                      key={m.id}
                      onClick={() => setEntrega(m.id)}
                      className={`flex items-center gap-3 p-4 rounded-xl border-2 text-left transition-all ${
                        metodoEntrega === m.id
                          ? 'border-brand-purple-600 bg-brand-purple-50'
                          : 'border-gray-200 hover:border-brand-purple-300'
                      }`}
                    >
                      <span className={metodoEntrega === m.id ? 'text-brand-purple-700' : 'text-gray-500'}>
                        {m.icon}
                      </span>
                      <div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input name="apellido" value={form.apellido} onChange={handleFormChange}
                      className="input" placeholder="Tu apellido" required />
                  </div>
                  <div>
                    ...
                    <input name="telefono" value={form.telefono} onChange={handleFormChange}
                      className="input" placeholder="11 XXXX-XXXX" type="tel" required />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 mb-1.5">Email</label>
                    <input name="email" value={form.email} onChange={handleFormChange}
                      className="input" placeholder="tu@email.com" type="email" />
                  </div>
                  {metodoEntrega === 'delivery' && (
                    <div className="sm:col-span-2">
                      ...
                      <input name="direccion" value={form.direccion} onChange={handleFormChange}
                        className="input" placeholder="Calle, número, piso/dpto" required />
                    </div>
                  )}
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-semibold text-gray-600 mb-1.5">Nota para el local</label>
                    <textarea name="nota" value={form.nota} onChange={handleFormChange}
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <p className="font-semibold text-sm text-gray-900">{m.label}</p>
                          {m.badge && (
                            <span className="badge-purple text-[10px]">{m.badge}</span>
                          )}
                        </div>
                        <p className="text-xs text-gray-500">{m.desc}</p>
                      </div>
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                        metodoPago === m.id ? 'border-brand-purple-600' : 'border-gray-300'
                      }`}>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sticky top-24">
            ...
            <div className="border-t border-gray-100 pt-3 space-y-2 mb-5">
              ...
              <div className="flex justify-between text-sm text-gray-600">
                ...
                <span className="text-green-600 font-medium">
                  {metodoEntrega === 'retiro' ? 'Gratis' : 'A consultar'}
                </span>
              </div>
            ...
            {paso === 2 && (
              <button onClick={() => setPaso(1)} className="w-full text-center text-gray-400 text-sm mt-3 hover:text-gray-600 transition-colors flex items-center justify-center gap-1">
