 ...
 * Schema exacto de la tabla productos:
 *   id, codigo, descripcion, precio, stock, unidad,
...
// Deduce categoría por palabras clave cuando categoria = 'General' o null
function deducirCategoria(descripcion) {
  ...
  if (/detergente|lavandina|jabón|shampoo|acondicionador|esponja|fosforo|vela |desodorante|cif |colgate|ola |skip |ariel|omo |rexona|nivea|pantene|head &|barbijo|papel hig/.test(n)) return 'Art. de Limpieza';
...
function normalizar(p) {
  // Si categoria es null o 'General', deducir por nombre
  const catRaw = (!p.categoria || p.categoria === 'General')
    ? deducirCategoria(p.descripcion)
    : p.categoria;

  const imagen = p.foto_url || IMAGEN_POR_CATEGORIA[catRaw] || IMAGEN_POR_CATEGORIA['General'];
